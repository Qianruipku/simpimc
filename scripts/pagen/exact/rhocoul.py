...
def RhoCoul(r1,r2,theta,tau,xkappa,z,kmax,lmax,nmax,D):
    ...
    rhocoul = 0.
    if D == 2:
        rhocoul = integrate.quad(CGrand2D, 0., 2.*uklim, args=(r1,r2,theta,tau,xkappa,z,lmax), limit=2**100)[0]
    elif D == 3:
        rhocoul = integrate.quad(CGrand3D, 0., 2.*uklim, args=(r1,r2,theta,tau,xkappa,z,lmax), limit=2**100)[0]
    if (z > 0.):
        return rhocoul
    else:
        if D == 2:
